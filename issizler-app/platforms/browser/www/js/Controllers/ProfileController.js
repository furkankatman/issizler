angular.module("issizlerApp").controller("ProfileController", function ($scope, $mdDialog) {
    ...
    $scope.UpdateProfile = function () {
        if ($scope.ProfileImageBlob != null) {
            ...
            imageRef.child(fileName).put($scope.ProfileImageBlob).then(function (snapshot) {
                snapshot.ref.getDownloadURL().then(function (downloadURLF) {
                    ...
                    firebase.auth().currentUser.updateProfile({photoURL:downloadURLF}).then(()=>{
        ...
        firebase.auth().currentUser.updateProfile({displayName:$scope.User.Username}).then(()=>{
            $scope.los.set("User",firebase.auth().currentUser);
        });
    }
